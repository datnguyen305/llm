...
if __name__ == "__main__":
    ...
    response = "Quyền phản đối được khôi phục bởi Orléans đã giúp Nghị viện nắm giữ quyền lực đáng kể, mặc dù nhà vua luôn duy trì quyền lực tuyệt đối và không bao giờ cho phép bất kỳ sự can thiệp nào từ Nghị viện."

    '''text_generator = TextGenerator(context, question)
    response = text_generator.generate_response()
    print("Generated Response:", response)'''

    # Sử dụng class mới
    evaluator = TextEvaluator(question=question, generated_answer=response)
